...
def Temperature (RawData, StationName, f, BaseData, StationExports, OmitYearsT, YearLabel):
    ...
    for aRow in BaseData:
        ...
        if Temp2 == 'nan':
            Check2.append(Temp2)
        # Check1.append(aRow[-1])
        # Check2.append(aRow[-2])

    tt1 = collections.Counter(Check1)
    tt2 = collections.Counter(Check2)

    if len(tt1.keys()) == 0 and len(tt2.keys()) == 0:
    ...
    if len(tt1.keys()) == 1 and len(tt2.keys()) == 1:
        z = 0
        while z < 102:
